...
public class PersonCollection : IPersonCollection
{
    ...
    {
        foreach (var pair in this.personByAge.Where(p => p.Key >= startAge && p.Key <= endAge))
        {
    ...
    public IEnumerable<Person> FindPersons(int startAge, int endAge, string town)
    {
        ...
        foreach (var pair in this.personByTownAndAge[town].Where(p => p.Key >= startAge && p.Key <= endAge))
        {
            foreach (var person in pair.Value)
            {
                yield return person;
